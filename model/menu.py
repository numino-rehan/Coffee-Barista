...
class Menu:
    ...
    def __str__(self):
        ...
        menu = self.get_menu()
        if not menu:
            return Fore.RED + "Menu is currently unavailable" + Style.RESET_ALL
            
        lines = [Fore.BLUE + Style.BRIGHT + "\nMENU:" + Style.RESET_ALL]
        for drink, val in menu.items():
            try:
                price = f"${val['cost']:.2f}"
                available = f"{Fore.GREEN}Yes" if val["in_stock"] else f"{Fore.RED}No"
                item_id = val["item_id"]
                lines.append(
                    f"{Fore.YELLOW}{item_id}{Style.RESET_ALL}: "
                    f"{Fore.CYAN} {drink} {Style.RESET_ALL},{Fore.MAGENTA} {price} {Style.RESET_ALL}, Available: {available}"
                )
            except KeyError as e:
                lines.append(f"{Fore.RED}Invalid menu item format: {e}{Style.RESET_ALL}")
